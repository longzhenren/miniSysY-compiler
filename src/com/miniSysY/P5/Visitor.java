...
public class Visitor extends P5BaseVisitor<Void> {
    ...
    //    public static HashMap<String, Integer> ident_Reg = new HashMap<>(); // 变量名到寄存器的映射
    public static HashMap<RuleContext, HashMap<String, Integer>> block_ident_Reg = new HashMap<>();
    public static HashMap<Integer, String> reg_Type = new HashMap<>();
    public static HashMap<String, Long> constIdent_Val = new HashMap<>();// 常量值表
    ...
    public boolean ident_Check_Reg_this_Block(RuleContext ctx, String Ident) {
        ...
        while (!(parent instanceof P5Parser.BlockContext)) {
...
//    }
    ...
    public void ident_Put_Reg(RuleContext ctx, String Ident, Integer Reg) {// 添加符号
        RuleContext parent = ctx;
        while (!(parent instanceof P5Parser.CompUnitContext)) {
            if (parent instanceof P5Parser.BlockContext) {
                HashMap<String, Integer> I_R;
                if (block_ident_Reg.containsKey(parent)) {
    ...
    public Integer ident_Get_Reg(RuleContext ctx, String Ident) {
        RuleContext parent = ctx;
        while (!(parent instanceof P5Parser.CompUnitContext)) {
            if (parent instanceof P5Parser.BlockContext) {
                if (block_ident_Reg.containsKey(parent)) {
...
//            Integer identReg = ident_Reg.get(ctx.Ident().getText());
            Integer identReg = ident_Get_Reg(ctx, Ident);
            int thisReg = currentReg++;
            reg_Type.put(thisReg, "i32");
            IR_List.add("\t%x" + thisReg + " = load i32, i32* %x" + identReg + "\n");
            attr_val.put("thisReg", thisReg);
        } else if (constIdent_Val.containsKey(Ident)) {
            attr_val.put("nodeVal", constIdent_Val.get(Ident));
        } else {
...
//            if (constIdent_Val.containsKey(cdf.Ident().getText()) || ident_Reg.containsKey(cdf.Ident().getText())) {
            if (constIdent_Val.containsKey(cdf.Ident().getText()) || ident_Check_Reg_this_Block(cdf, cdf.Ident().getText())) {
                System.err.println("Re-Declaration!");
                System.exit(1);
            }
            visit(cdf);
        }
...
//            if (ident_Reg.containsKey(Ident) || constIdent_Val.containsKey(Ident)) {
            if (ident_Check_Reg_this_Block(ctx, Ident) || constIdent_Val.containsKey(Ident)) {
                System.err.println("Re-Declaration!");
                System.exit(1);
            }
            visit(vdf);
...
//                if (!ident_Reg.containsKey(Ident) || constIdent_Val.containsKey(Ident)) {
                if (!ident_Check_Reg(ctx, Ident) || constIdent_Val.containsKey(Ident)) {
                    System.err.println("Undeclared Ident:" + Ident);
                    System.exit(1);
                }
...
//            if (ident_Reg.containsKey(Ident) || constIdent_Val.containsKey(Ident) || !Main.declaredFunc.containsKey(Ident))
            if (ident_Check_Reg_this_Block(ctx, Ident) || constIdent_Val.containsKey(Ident) || !Main.declaredFunc.containsKey(Ident))
                System.exit(1);
    ...
    public Void visitCond(P5Parser.CondContext ctx) {
        ...
        if (node_Attr_Val.get(ctx.lOrExp()).containsKey("numberVal")) {
            ...
            IR_List.add("\t%x" + thisReg + " = icmp ne i32 " + numberVal + ", 0\n");
    ...
    public Void visitLAndExp(P5Parser.LAndExpContext ctx) {
        ...
        if (ctx.LAND_KW() == null) { // eqExp
            ...
            visit(ctx.lAndExp());
            Integer lAndExpReg, eqExpReg, reg1 = null, reg2 = null;
            if (node_Attr_Val.get(ctx.lAndExp()).containsKey("thisReg")) {
                ...
                if (reg_Type.get(lAndExpReg).equals("i1")) {
                    ...
                    IR_List.add("\t%x" + reg1 + " = icmp ne i32 %x" + lAndExpReg + ", 0" + "\n");
                }
            }
            visit(ctx.eqExp());
            if (node_Attr_Val.get(ctx.eqExp()).containsKey("thisReg")) {
                ...
                if (reg_Type.get(eqExpReg).equals("i1")) {
                    ...
                    IR_List.add("\t%x" + reg2 + " = icmp ne i32 %x" + eqExpReg + ", 0" + "\n");
                }
            }
            Integer thisReg = currentReg++;
            reg_Type.put(thisReg, "i1");
    ...
    public Void visitLOrExp(P5Parser.LOrExpContext ctx) {
        ...
        if (ctx.LOR_KW() == null) { // lAndExp
            ...
            visit(ctx.lOrExp());
            Integer lOrExpReg, lAndExpReg, reg1 = null, reg2 = null;
            if (node_Attr_Val.get(ctx.lOrExp()).containsKey("thisReg")) {
                ...
                if (reg_Type.get(lOrExpReg).equals("i1")) {
                    ...
                    reg1 = currentReg++;
                    reg_Type.put(reg1, "i1");
                    IR_List.add("\t%x" + reg1 + " = icmp ne i32 %x" + lOrExpReg + ", 0" + "\n");
                }
            }
            visit(ctx.lAndExp());
            if (node_Attr_Val.get(ctx.lAndExp()).containsKey("thisReg")) {
                ...
                if (reg_Type.get(lAndExpReg).equals("i1")) {
                    ...
                    reg2 = currentReg++;
                    reg_Type.put(reg2, "i1");
                    IR_List.add("\t%x" + reg2 + " = icmp ne i32 %x" + lAndExpReg + ", 0" + "\n");
                }
            }
            Integer thisReg = currentReg++;
            reg_Type.put(thisReg, "i1");
